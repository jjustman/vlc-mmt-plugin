...
typedef union  {
	mmtp_packet_header_fields_t					mmtp_packet_header;
	...
	generic_object_fragments_t 					mmtp_generic_object_fragments;
	signalling_message_fragments_t				mmtp_signalling_message_fragments;
	repair_symbol_t								mmtp_repair_symbol;
...
void mmtp_sub_flow_push_mmtp_packet(mmtp_sub_flow_t *mmtp_sub_flow, mmtp_packet_header_fields_t *entry) {
	...
	if(entry->mmtp_payload_type == 0x00) {
		...
		vlc_vector_push(&mpu_fragments->all_mpu_fragments_vector, entry);

	} else if(entry->mmtp_payload_type == 0x01) {
		vlc_vector_push(&mmtp_sub_flow->mmtp_generic_object_fragments_vector, entry);
	} else if(entry->mmtp_payload_type == 0x02) {
		vlc_vector_push(&mmtp_sub_flow->mmtp_signalling_message_fragements_vector, entry);
	} else if(entry->mmtp_payload_type == 0x03) {
		vlc_vector_push(&mmtp_sub_flow->mmtp_repair_symbol_vector, entry);
	}
}
